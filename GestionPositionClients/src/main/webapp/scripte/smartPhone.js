$(document).ready(function() {
	...
	$("#content").on("click", '.badge', function() {
		...
		if ($(this).attr("data-role") === "delete") {
			...
			$.ajax({
				...
				success: function(data) {
					console.log(data);
					$("#inputUsers").val(data.utilisateur.id);
					$("#imei").val(data.imei);
					$("#id").val(data.id);
				},
				error: function(data) {
	...
	function listerSpc(data) {
		...
		data.forEach(function(e) {
			...
			console.log(e);
			ligne += "<tr><th scope='row'>" + e.id + "</th><td value='" + e.utilisateur.nom + "' >" + e.utilisateur.prenom + "</td><td valuel=" + e.imei + ">" + e.imei
				+ "</td><td><button indice=" + e.id + " data-role='delete' class='badge badge-danger'>Supprimer</button>&nbsp;&nbsp;&nbsp;<button indice=" + e.id
				+ " data-role='update' class='badge badge-info'>Modifier</button>";
		});
		$("#content").html(ligne);
	}
});
